...
class Server
{
    const OPTION_SERVER = 1;
    const OPTION_MULTI = 2;
    const OPTION_REQUEST = 4;
    const OPTION_REPLY = 8;
    const OPTION_WAIT = 16;
    const OPTION_STATUS = 32;
    const OPTION_HEARTBEAT = 64;
    const OPTION_CLOSE = 128;
    ...
    {
        if(($kind & self::OPTION_SERVER)){//as server
            if(!isset($this->server[$conn->id])){
    ...
    public function handleRequest($kind, Connection $conn, $message)
    {
        ...
        $this->log('request');
//        $that = $this;
//        $watcher = new \EvTimer(0.5, function() use ($that, $watcher, $md5, $nullReply){
//            $watcher->stop();
...
//        $this->watchers[$md5] = $watcher;
    ...
    {
        $data = $this->codec->serialize(array($kind, $seqId, $message));
    ...
    public function process($clientSocket)
    {
        ...
        $watcher = new \EvIo($clientSocket, \Ev::READ, function() use ($that, $id){
            ...
            if(is_string($str) && strlen($str)){
                ...
                $that->write($this->connections[$id], $str);
                $this->log('write response finished');
            }
